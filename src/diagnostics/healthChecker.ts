...
function isWindowsEnvironmentError(input: string): boolean {
    return input.includes(':\\\\') || input.includes('\\\\Users\\\\') || input.includes('系统找不到');
}
...
async function checkMgraftcp(extensionPath?: string): Promise<DiagnosticCheck> {
    ...
    try {
        ...
        if (isWindowsEnvironmentError(homeDir)) {
            ...
            check.suggestion = 'Please install "SSH Relay Guard" on the remote server: Open Extensions (Ctrl+Shift+X), search "SSH Relay Guard", click "Install in SSH: <host>".';
            return check;
        }
    ...
    try {
        ...
        if (!trustedIP) {
            ...
            if (isLikelyGoogle) {
                ...
                check.suggestion = 'Cannot verify against trusted DNS (8.8.8.8 blocked). Ensure mgraftcp-fakedns is working for LS traffic.';
            }
            return check;
        }
        ...
        if (systemIP === trustedIP) {
            ...
            if (systemIsGoogle && trustedIsGoogle) {
                ...
                check.suggestion = 'DNS is poisoned — but each layer already has a bypass: ' +
 ...
 */
export async function runDiagnostics(configService: ConfigService, onProgress?: ProgressCallback, extensionPath?: string): Promise<DiagnosticReport> {
    ...
    if (isLocal) {
        ...
        const REMOTE_ONLY_IDS = new Set(['remote-forward', 'mgraftcp', 'ls-wrapper', 'ls-process', 'external-connectivity', 'dns-pollution']);
        checks.forEach(c => {
        ...
        [2, 3, 5, 7].forEach(i => { checks[i].status = 'running'; });
        onProgress?.(checks);
        ...
        [4, 6].forEach(i => { checks[i].status = 'running'; });
        onProgress?.(checks);
