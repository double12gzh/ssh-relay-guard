...
import { isMgraftcpRunning, getMonitoredProcess, killTargetProcess, promptReloadWindow, customExecAsyncForTesting, invalidateProcessCache } from '../utils/processUtils';
...
suite('processUtils Tests', () => {
    ...
    test('getMonitoredProcess should correctly parse ps aux output', async () => {
        const psOutput = 'user 12345 0.0 0.1 2000 1000 ? S 12:00 0:00 language_server_linux /path/to/server.js\n';
        (customExecAsyncForTesting as any) = async () => {
    ...
    test('getMonitoredProcess should detect isUsingProxy when mgraftcp is in command', async () => {
        const psOutput = 'user 12345 0.0 0.1 2000 1000 ? S 12:00 0:00 mgraftcp language_server_linux /path/to/server.js\n';
        (customExecAsyncForTesting as any) = async () => {
            return { stdout: psOutput, stderr: '' };
        };
