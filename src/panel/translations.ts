...
const zh = {
	...
	tipNoteLocal:
		'首次安装后需要先运行「Add Host Forwarding」来配置 SSH 隧道，否则远程端无法连通！',
	tipTitleRemote: '故障排除',
	...
	tunnelDisconnected:
		'⚠️ SSH 隧道已断开！代理不可达，本地端 autossh 将尝试自动重连。如长时间未恢复，请检查本地网络和代理软件。',
	autosshNotInstalled:
		'autossh 未安装，隧道断开后需要手动重连。建议运行 brew install autossh (macOS) 或 apt install autossh (Linux) 安装。',
};
...
const en: Translations = {
	...
	tunnelStep3: 'Click "Refresh Status" below to verify the tunnel is working',
	tunnelStep4: 'If still failing, close remote and reconnect from a local window',
	retryCheck: 'Refresh Status',
	...
	tipStep3Local:
		'Connect to your remote server — the SSH tunnel will be established automatically',
	tipStep4Local: 'Install SRG extension on the remote side → Reload remote window if prompted',
	tipNoteLocal:
		'After fresh install, you must run "Add Host Forwarding" to configure the SSH tunnel first!',
	...
	tipNoteRemote:
		'This usually happens when you connect directly to the remote without opening a local window first.',
	pending: 'Pending',
	success: 'Pass',
	warning: 'Warning',
	error: 'Error',
	...
	rollbackDesc:
		'Restores previous Language Server configuration. Use only if the plugin causes critical issues or before uninstalling.',
	localPortTip:
	...
	remotePortTipRemote:
		'🔒 These settings are managed by the local side. To change, adjust in the Local SRG panel',
	tunnelDisconnected:
		'⚠️ SSH tunnel disconnected! Proxy unreachable. autossh on your local machine will attempt to reconnect. If the issue persists, check your local network and proxy software.',
	autosshNotInstalled:
		'autossh is not installed. When the tunnel disconnects, manual reconnection will be needed. Install it with: brew install autossh (macOS) or apt install autossh (Linux).',
};
